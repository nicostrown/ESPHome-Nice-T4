...
• ADR: This is the address of the NICE network where the devices you want to manage are located. This can be a value
from 1 to 63 (1 to 3F). This value must be in HEX. If the destination is an integration module on DIN-BAR, this value is
0 (adr = 0) if the destination is is an intelligent engine, this value is 1 (adr = 1). • EPT: This is the address of the
Nice engine included in the network ADR. It can be a value between 1 and 127. This value must be in HEX. • CMD: This is
the command you want to send to the destination (ADR, EPT). • PRF: profile setup command. • FNC: This is the function
you want to send to the destination (ADR, EPT). • EVT: This is the event that is sent to the destination (ADR, EPT).
*/
...
class NiceCover : public cover::Cover, public uart::UARTDevice, public Component {
 public:
  void setup() override;
  void loop() override;
  ...
  */
  void set_update_interval(uint32_t update_interval) { this->status_update_interval = update_interval; }
 ...
 protected:
  ...
  std::vector<uint8_t> raw_cmd_prepare(
      std::string data);  // preparation of user-entered data for the possibility of sending

  // inf command generation
  std::vector<uint8_t> gen_inf_cmd(uint8_t to_addr1, uint8_t to_addr2, uint8_t whose, uint8_t inf_cmd, uint8_t run_cmd,
                                   uint8_t next_data, const std::vector<uint8_t> &data, size_t len);
  // for commands without data
  std::vector<uint8_t> gen_inf_cmd(const uint8_t whose, const uint8_t inf_cmd, const uint8_t run_cmd) {
    return gen_inf_cmd((uint8_t) (this->to_addr >> 8), (uint8_t) (this->to_addr & 0xFF), whose, inf_cmd, run_cmd, 0x00,
                       {0x00}, 0);
  }
  // for commands with data
  std::vector<uint8_t> gen_inf_cmd(const uint8_t whose, const uint8_t inf_cmd, const uint8_t run_cmd,
                                   const uint8_t next_data, std::vector<uint8_t> data) {
    return gen_inf_cmd((uint8_t) (this->to_addr >> 8), (uint8_t) (this->to_addr & 0xFF), whose, inf_cmd, run_cmd,
                       next_data, data, data.size());
  }
