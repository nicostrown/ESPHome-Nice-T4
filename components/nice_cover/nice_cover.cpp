...
void NiceCover::loop() {
  ...
  if (this->gate_class == UNKNOWN) {
    ...
    if (this->status_update_interval && this->tx_buffer.empty() &&
        (millis() - this->last_received_status_millis) > this->status_update_interval) {
      this->query_status();
    }
  }
...
void NiceCover::handle_received_byte(uint8_t c) {
...
bool NiceCover::validate_received_message() {
...
void NiceCover::parse_status_packet(const std::vector<uint8_t> &data) {
  ...
  if ((data[1] == (data[12] + 0xd)) && (data[13] == NOERR)) {  // if evt
    ...
    if ((data[6] == INF) && (data[9] == FOR_CU) && (data[11] == GET - 0x80) && (data[13] == NOERR)) {
      ...
      switch (data[10]) {
        ...
        case POS_MAX:
          if (((data[14] << 8) + data[15]) >
              0x00) {  // if the response from the actuator contains data on the opening position
        ...
        case CUR_POS:
          ...
          ESP_LOGI(TAG, "Current gate position: %d, position in %%: %f", _pos_usl,
    ...
    if ((data[6] == INF) && (data[9] == FOR_CU) && (data[11] == SET - 0x80) &&
        (data[13] == NOERR)) {  // interested in responses to SET requests that came without errors from the drive
    ...
    if ((data[6] == INF) && (data[9] == FOR_ALL) && (data[11] == GET - 0x80) &&
        (data[13] == NOERR)) {  // interested in FOR_ALL responses to GET requests that came without errors
      ...
      switch (data[10]) {
        case MAN:
          ...
          break;
        case PRD:
          if (((uint8_t) (this->oxi_addr >> 8) == data[4]) &&
              ((uint8_t) (this->oxi_addr & 0xFF) == data[5])) {  // if the packet is from the receiver
            // ESP_LOGCONFIG(TAG, "  Receiver: %S ", str.c_str());
            this->oxi_product.assign(this->rx_buffer.begin() + 14, this->rx_buffer.end() - 2);
          } else if (((uint8_t) (this->to_addr >> 8) == data[4]) &&
                     ((uint8_t) (this->to_addr & 0xFF) == data[5])) {  // if the package is from the drive controller
    ...
    if ((data[9] == 0x0A) && (data[10] == 0x25) && (data[11] == 0x01) && (data[12] == 0x0A) && (data[13] == NOERR)) {
      ESP_LOGCONFIG(TAG, "Remote Number: %X%X%X%X, Command: %X, Button: %X, Mode: %X, Hit Count: %d", vec_data[5],
                    vec_data[4], vec_data[3], vec_data[2], vec_data[8] / 0x10, vec_data[5] / 0x10, vec_data[7] + 0x01,
                    vec_data[6]);
    }
    ...
    if ((data[9] == 0x0A) && (data[10] == 0x26) && (data[11] == 0x41) && (data[12] == 0x08) && (data[13] == NOERR)) {
      ESP_LOGCONFIG(TAG, "Button %X, remote control number: %X%X%X%X", vec_data[0] / 0x10, vec_data[0] % 0x10,
                    vec_data[1], vec_data[2], vec_data[3]);
    }
  } else if ((data[14] == NOERR) &&
             (data[1] > 0x0d)) {  // otherwise, the Responce packet - confirmation of the received command
      ...
      case FOR_CU:
        ...
        switch (data[10] + 0x80) {
          ...
          case STA:
            ...
            this->position = (_pos_usl - _pos_cls) * 1.0f / (_pos_opn - _pos_cls);
            ESP_LOGD(TAG, "Current gate position: %d, position in %%: %f", _pos_usl,
                     (_pos_usl - _pos_cls) * 100.0f / (_pos_opn - _pos_cls));
            this->publish_state_if_changed();
            this->last_received_status_millis = millis();
            break;
...
std::vector<uint8_t> NiceCover::gen_control_cmd(const uint8_t control_cmd) {
  std::vector<uint8_t> frame = {(uint8_t) (this->to_addr >> 8), (uint8_t) (this->to_addr & 0xFF),
                                (uint8_t) (this->from_addr >> 8), (uint8_t) (this->from_addr & 0xFF)};  // header
...
std::vector<uint8_t> NiceCover::gen_inf_cmd(const uint8_t to_addr1, const uint8_t to_addr2, const uint8_t whose,
                                            const uint8_t inf_cmd, const uint8_t run_cmd, const uint8_t next_data,
                                            const std::vector<uint8_t> &data, size_t len) {
  std::vector<uint8_t> frame = {to_addr1, to_addr2, (uint8_t) (this->from_addr >> 8),
                                (uint8_t) (this->from_addr & 0xFF)};  // заголовок
...
void NiceCover::send_raw_cmd(std::string data) {
  std::vector<uint8_t> v_cmd = raw_cmd_prepare(data);
  send_array_cmd(&v_cmd[0], v_cmd.size());
}

std::vector<uint8_t> NiceCover::raw_cmd_prepare(std::string data) {
  // remove everything except letters and numbers
  data.erase(remove_if(data.begin(), data.end(), [](const unsigned char ch) { return (!(iswalnum(ch))); }), data.end());
...
void NiceCover::send_array_cmd(std::vector<uint8_t> data) {
  return send_array_cmd((const uint8_t *) data.data(), data.size());
}

void NiceCover::send_array_cmd(const uint8_t *data, size_t len) {
  char br_ch = 0x00;
  this->flush();
  ...
  delayMicroseconds(520);    // add a delay to the wait, otherwise the speed will switch before sending. With a delay on
...
void NiceCover::send_inf_cmd(std::string to_addr, std::string whose, std::string command, std::string type_command,
                             std::string next_data, bool data_on, std::string data_command) {
  std::vector<uint8_t> v_to_addr = raw_cmd_prepare(to_addr);
  std::vector<uint8_t> v_whose = raw_cmd_prepare(whose);
  std::vector<uint8_t> v_command = NiceCover::raw_cmd_prepare(command);
  ...
  if (data_on) {
    tx_buffer.push(gen_inf_cmd(v_to_addr[0], v_to_addr[1], v_whose[0], v_command[0], v_type_command[0], v_next_data[0],
                               v_data_command, v_data_command.size()));
  } else {
    tx_buffer.push(
        gen_inf_cmd(v_to_addr[0], v_to_addr[1], v_whose[0], v_command[0], v_type_command[0], v_next_data[0]));
  }
}

void NiceCover::set_mcu(std::string command, std::string data_command) {
...
void NiceCover::init_device(const uint8_t addr1, const uint8_t addr2, const uint8_t device) {
  if (device == FOR_CU) {
...
bool NiceCover::supports_querying_position() const { return this->_max_opn > 0; }

void NiceCover::query_status() {
...
void NiceCover::publish_state_if_changed() {
